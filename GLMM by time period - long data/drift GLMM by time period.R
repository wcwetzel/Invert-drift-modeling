...
m0 = lmer(cbind(drift, stay) ~ (1|rep) + time + I(time^2) + velocity, data=dldata, family='binomial')

m1 = lmer(cbind(drift, stay) ~ (1|rep) + initial + time + I(time^2), data=dldata, family='binomial')

m2 = lmer(cbind(drift, stay) ~ (1|rep) + initial + time + I(time^2) + 
	velocity, data=dldata, family='binomial')

m3 = lmer(cbind(drift, stay) ~ (1|rep) + time + I(time^2)
	+ initial, data=dldata, family='binomial')

m4 = lmer(cbind(drift, stay) ~ (1|rep) + time + I(time^2)
	+ initial + velocity, data=dldata, family='binomial')

### seems strange to have initial in here because i
### would think it's contained in drift stay.
### it's their sum. hm... ask mcE?
...
m0 = lmer(cbind(drift, stay) ~ (1|rep) + time + I(time^2), data=fldata, family='binomial')
...
m4.9 = lmer(cbind(drift, stay) ~ (1|rep) + initial + food + I(food^2) +
	time + I(time^2), data=fldata, family='binomial')
...
AICctab(logLik(m0), logLik(m1), logLik(m2), logLik(m3), logLik(m4), logLik(m4.5),
	logLik(m5), logLik(m4.7), logLik(m4.8), logLik(m4.9), weights=TRUE, nobs=nrow(fldata)/4)
